#!/usr/bin/env python3
"""Live check of WorkspaceRules.qml against the running Hyprland.
...
The instance runs with private XDG data/state dirs, so nothing of the user's
is read or written, and with no session bus (DBUS_SESSION_BUS_ADDRESS is an
unparseable address, never unset: unset, libdbus may autolaunch the throwaway
bus that docs/LESSONS.md forbids): Hyprland's IPC is a Unix socket and the
rules need nothing from D-Bus, so Qt's platform handshake is skipped with it.
"""
import os
import shutil
import subprocess
...
ShellRoot {
  Plugin.WorkspaceRules {
    ...
    Component.onCompleted: {
      ...
      console.log("RULES pin script:", pinScript("address:0x5642f0a1b2c3", true, 320, 240).split("\\n").length, "lines")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="dn-rules-") as tmp:
        d = Path(tmp)
        env = dict(os.environ, XDG_DATA_HOME=str(d / "data"), XDG_STATE_HOME=str(d / "state"),
                   XDG_CACHE_HOME=str(d / "cache"), DBUS_SESSION_BUS_ADDRESS="disabled:",
                   QT_QPA_PLATFORMTHEME="generic", NO_AT_BRIDGE="1")
        shutil.copy(ROOT / "WorkspaceRules.qml", d)
        shutil.copy(ROOT / "RulesLua.js", d)
