# DeskNotes for Omarchy

Sticky notes as ordinary Hyprland windows, drawn by `omarchy-shell` itself. No browser engine,
...
```sh
omarchy plugin add https://github.com/lolu13/desknotes-omarchy.git
cd ~/.config/omarchy/plugins/lolu13.desknotes
...
2. writes `~/.config/hypr/desknotes.lua` (one window rule, five bindings) and one `dofile` line at the end of
   `~/.config/hypr/bindings.lua`, validated with `hyprctl configerrors` and rolled back on any error,
3. copies the built helper to `~/.local/bin/desknotes-helper`, a launcher entry to
   `~/.local/share/applications/desknotes.desktop` and an icon to `~/.local/share/icons/hicolor/128x128/apps/`,
4. enables the plugin and restarts the shell.

Every file it replaces is backed up first under `~/.local/state/desknotes-omarchy/install-backups/`.
...
is shipped. `desknotes-helper` must stay in `~/.local/bin` because `omarchy plugin update` replaces the
plugin directory.
...
| Copy the selected note as Markdown / save it to `~/Documents/DeskNotes/` | Ctrl+E / Ctrl+Shift+E in the library |

Bar button: left click opens Notes & Stack, middle click creates a note, right click stacks all.
The first launch with an empty database creates a pinned "Welcome to DeskNotes" note with these keys;
`omarchy-shell desknotes welcome` brings it back.

Command line: `omarchy-shell desknotes newNote | hideAll | restoreAll | pinNote | newNoteFromClipboard
| settings | welcome | mirror | mirrorStatus | status | reload` and `omarchy-shell shell toggle lolu13.desknotes`.
...
- `~/.local/share/com.desknotes.omarchy/desknotes.db` (plus SQLite's `-wal`/`-shm` files): the notes.
...
- `~/Documents/DeskNotes/<title>.md` on Ctrl+Shift+E, never replacing an existing file (` (2)`, ` (3)`, …).
...
```sh
python3 ~/.config/omarchy/plugins/lolu13.desknotes/scripts/uninstall.py
omarchy plugin remove lolu13.desknotes
```

The uninstall script disables the plugin, removes `~/.local/bin/desknotes-helper`, the launcher entry, the
icon, `~/.config/hypr/desknotes.lua` and the one `dofile` line in `bindings.lua` (validated with
...
- the Markdown mirror folder and `~/Documents/DeskNotes/` exports,
- `~/.local/state/desknotes-omarchy/install-backups/`,
- the bar entry in `~/.config/omarchy/shell.json` (remove DeskNotes from the bar in the shell settings).

## Layout

```
manifest.json, *.qml, *.js, blocks/   the plugin, loaded by omarchy-shell
helper/                                Rust: desknotes-helper, JSON lines over stdin/stdout
hypr/desknotes.lua                     window rule + bindings, installed as ~/.config/hypr/desknotes.lua
...
- `omarchy-shell desknotes status` returns `{ready, open, stacked, unsaved, error}`.
- Tests: `cargo test --release --manifest-path helper/Cargo.toml`, `python3 helper/tests/smoke.py`
  (a throwaway database; set `DESKNOTES_DB=/abs/path` to use a copy), `python3 scripts/test-qml.py`.
