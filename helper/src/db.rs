//! The database layer: notes, tabs, settings and search over the SQLite file at
//! ~/.local/share/com.desknotes.omarchy/desknotes.db. It started as the DeskNotes
//! desktop edition's db.rs and keeps that schema, so notes written by either stay
//! readable. It only depends on rusqlite and serde. Some functions are not called
//! by the helper yet.
#![allow(dead_code)]
...
impl Database {
    ...
    /// Every row move or none: a folder switch must not leave some notes
    /// tracked in the new folder and the rest in the old one.
    pub fn set_mirror_paths(&self, moves: &[(String, String)]) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
        for (note_id, path) in moves {
            tx.execute(
    ...
    #[cfg(test)]
    pub(crate) fn execute_for_tests(&self, sql: &str) {
    ...
    /// Forgets every mirrored file (the files stay): the tracking rows say
    /// which files a folder switch may move away, and a folder the mirror
    /// left is not a folder it is switching from.
    pub fn clear_mirror_paths(&self) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        conn.execute("DELETE FROM mirror_files", []).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    /// Tabs of open notes only; a stacked note's tabs are fetched with `tabs_for`
    /// when it is restored, so a resync never carries every tab body at once.
    pub fn list_open_tabs(&self) -> Result<Vec<NoteTab>, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let sql = format!(
            "SELECT {} FROM note_tabs t JOIN notes n ON n.id = t.note_id
             WHERE n.piled = 0 ORDER BY t.note_id, t.position",
            TAB_COLS.split(", ").map(|c| format!("t.{c}")).collect::<Vec<_>>().join(", ")
        );
        let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
        let rows = stmt.query_map([], row_to_tab).map_err(|e| e.to_string())?;
        rows.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())
    ...
    /// Visits the tabs of open notes one row at a time, in `list_open_tabs`
    /// order: the visitor keeps what it needs and the row's body is dropped
    /// before the next is read, so a collection of large tabs never sits in
    /// memory whole.
    pub fn for_each_open_tab(&self, mut f: impl FnMut(NoteTab) -> Result<(), String>) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let sql = format!(
            "SELECT {} FROM note_tabs t JOIN notes n ON n.id = t.note_id
             WHERE n.piled = 0 ORDER BY t.note_id, t.position",
        ...
        let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
        let tabs = stmt.query_map([], row_to_tab).map_err(|e| e.to_string())?;
        for tab in tabs {
            f(tab.map_err(|e| e.to_string())?)?;
        }
        Ok(())
    }
    ...
    /// A tab created, edited or deleted is a change to its note: the note's
    /// `updated_at` moves with it (the mirror's front matter and the desktop's
    /// ordering read the note's, not the tab's).
    const TOUCH_NOTE: &'static str = "UPDATE notes SET updated_at = datetime('now') WHERE id = ?1";
    ...
    pub fn create_tab(&self, id: &str, note_id: &str) -> Result<NoteTab, String> {
        ...
        .map_err(|e| e.to_string())?;
        tx.execute(Self::TOUCH_NOTE, params![note_id]).map_err(|e| e.to_string())?;
        let sql = format!("SELECT {TAB_COLS} FROM note_tabs WHERE id = ?1");
        let tab = tx.query_row(&sql, params![id], row_to_tab).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())?;
        Ok(tab)
    }
    ...
    pub fn update_tab(&self, tab: &NoteTab) -> Result<(), String> {
        ...
        tx.execute(
            "UPDATE notes SET updated_at = datetime('now') WHERE id = (SELECT note_id FROM note_tabs WHERE id = ?1)",
            params![tab.id],
        )
        .map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())
    ...
    /// Deletes a tab, closes the gap in its note's positions and drops the
    /// tab's per-note settings (`note.<note>.<anything>.<tab>`), atomically.
    pub fn delete_tab(&self, id: &str) -> Result<(), String> {
        ...
        .map_err(|e| e.to_string())?;
        tx.execute(
            "DELETE FROM app_settings WHERE substr(key, 1, length(?1)) = ?1 AND substr(key, -length(?2)) = ?2",
            params![format!("note.{note_id}."), format!(".{id}")],
        )
        .map_err(|e| e.to_string())?;
        tx.execute(Self::TOUCH_NOTE, params![note_id]).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())
    ...
    /// Deletes a note with its tabs (cascade) and every `note.<id>.*` setting
    /// (titles, tab labels, layout) the shell stored for it, atomically.
    pub fn delete_note_with_settings(&self, id: &str) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM notes WHERE id = ?1", params![id]).map_err(|e| e.to_string())?;
        tx.execute(
    ...
    pub fn search_note_ids_fts(&self, fts_query: &str) -> Result<Vec<String>, String> {
        ...
        let mut tabs = conn
            .prepare_cached(
                // One row per note (its best tab decides the order), so a note
                // with many matching tabs cannot use up the limit alone.
                "SELECT t.note_id FROM tabs_fts t JOIN notes n ON n.id = t.note_id \
                 WHERE tabs_fts MATCH ?1 GROUP BY t.note_id ORDER BY min(t.rank) LIMIT 200",
            )
            .map_err(|e| e.to_string())?;
        let hits = tabs
    ...
    pub fn create_notes(&self, notes: &[Note]) -> Result<(), String> {
        self.create_notes_with_tabs(notes, &[])
    }

    /// Notes and their extra tabs (an import) in one transaction: a tab whose
    /// note is missing, or any other failure, leaves nothing behind.
    pub fn create_notes_with_tabs(&self, notes: &[Note], tabs: &[NoteTab]) -> Result<(), String> {
        for note in notes {
            validate_note_storage(note)?;
        }
        if tabs.iter().any(|tab| tab.content_blocks.len() > MAX_NOTE_CONTENT_BYTES) {
            return Err("Note content exceeds 5 MB".into());
        }
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
        ...
        for tab in tabs {
            tx.execute(
                "INSERT INTO note_tabs (id, note_id, position, icon, content_blocks) VALUES (?1, ?2, ?3, ?4, ?5)",
                params![tab.id, tab.note_id, tab.position, tab.icon, tab.content_blocks],
            )
            .map_err(|e| e.to_string())?;
        }
        tx.commit().map_err(|e| e.to_string())
    ...
    /// Visits every note, open ones first (by z-order) then stacked ones
    /// (newest first), one row at a time: the visitor keeps what it needs and
    /// the row's body is dropped before the next is read, so a collection of
    /// large notes never sits in memory whole.
    pub fn for_each_note(&self, mut f: impl FnMut(Note) -> Result<(), String>) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        for sql in [
            format!("SELECT {} FROM notes WHERE piled = 0 ORDER BY z_order ASC", SELECT_COLS),
            format!("SELECT {} FROM notes WHERE piled = 1 ORDER BY updated_at DESC", SELECT_COLS),
        ] {
            let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
            let notes = stmt.query_map([], row_to_note).map_err(|e| e.to_string())?;
            for note in notes {
                f(note.map_err(|e| e.to_string())?)?;
            }
        }
        Ok(())
    }

    pub fn has_any_note(&self) -> Result<bool, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        conn.query_row("SELECT EXISTS(SELECT 1 FROM notes)", [], |r| r.get::<_, bool>(0))
            .map_err(|e| e.to_string())
    }
    ...
    /// A setting the Omarchy edition owns in the shared database: `note.<id>.*`
    /// (titles, tab labels, layouts), `onote.*`, the mirror folder and the
    /// welcome flag. The desktop edition neither resets, imports nor writes
    /// them; the mirror folder in particular changes only through the helper's
    /// `setMirrorDir`, which forgets the tracked files first.
    pub fn is_plugin_setting(key: &str) -> bool {
        key.starts_with("note.") || key.starts_with("onote.") || key == "markdownMirrorDir" || key == "welcomeCreated"
    }
    ...
    /// Replaces the desktop edition's settings. Keys the Omarchy edition owns
    /// in the shared database (`note.<id>.*`, `onote.*`, the mirror folder,
    /// the welcome flag) are not the desktop's to reset or import over.
    pub fn replace_all_settings(&self, settings: &std::collections::HashMap<String, String>) -> Result<(), String> {
        let mut conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.transaction().map_err(|e| e.to_string())?;
        tx.execute(
            "DELETE FROM app_settings WHERE NOT (key LIKE 'note.%' OR key LIKE 'onote.%'
                                                OR key IN ('markdownMirrorDir', 'welcomeCreated'))",
            [],
        )
        .map_err(|e| e.to_string())?;
        {
            let mut stmt = tx
                ...
                .map_err(|e| e.to_string())?;
            for (key, value) in settings {
                // A backup written by this edition carries none of these; a
                // file that does must not write over what the delete kept.
                if Self::is_plugin_setting(key) {
                    continue;
                }
                stmt.execute(params![key, value])
    ...
    pub fn icon_pack_is_referenced(&self, pack_id: &str) -> Result<bool, String> {
        ...
        conn.query_row(
            "SELECT EXISTS(
                ...
                SELECT 1 FROM note_tabs WHERE instr(content_blocks, ?1) > 0
            )",
            params![pack_id],
    ...
    /// Deletes every note with its tabs (cascade) and every `note.<id>.*`
    /// setting the shell stored for them, atomically, like `delete_note_with_settings`.
    pub fn delete_all_notes(&self) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM notes", []).map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM app_settings WHERE key LIKE 'note.%'", []).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())
    }
}
...
mod tests {
    ...
    fn tabs_append_renumber_cascade_and_search() {
        ...
        // Many matching tabs on one note do not push another note's only match out.
        for i in 0..220 {
            let mut t = db.create_tab(&format!("many{i}"), "a").unwrap();
            t.content_blocks = r#"[{"type":"text","content":"pineapple again"}]"#.into();
            db.update_tab(&t).unwrap();
        }
        let mut tb = db.create_tab("tb", "b").unwrap();
        tb.content_blocks = r#"[{"type":"text","content":"pineapple on b"}]"#.into();
        db.update_tab(&tb).unwrap();
        assert_eq!(db.search_note_ids_fts("pineapple*").unwrap(), vec!["a", "b"]);
        for i in 0..220 {
            db.delete_tab(&format!("many{i}")).unwrap();
        }
        db.delete_tab("tb").unwrap();
    ...
    fn icon_pack_reference_query_checks_content_and_title_icons() {
        ...
        // A reference held only by an extra tab (an imported archive) counts too.
        db.create_notes_with_tabs(
            &[note_with_icon_reference("note-c", "[]", None)],
            &[NoteTab {
                id: "tab-c".into(), note_id: "note-c".into(), position: 1, icon: String::new(),
                content_blocks: r#"[{"content":"![icon](asset://localhost/icon-packs/pack-c/icon.png)"}]"#.into(),
        ...
        assert!(!db.icon_pack_is_referenced("pack-d").unwrap());
    }
    ...
    #[test]
    fn notes_import_with_their_tabs_or_not_at_all() {
        let db = test_db();
        let tab = |id: &str, note_id: &str, position: i32| NoteTab {
            id: id.into(),
        ...
        assert!(db.create_notes_with_tabs(&[note("n")], &[tab("t", "n", 1), tab("stray", "ghost", 1)]).is_err());
        assert!(db.get_note("n").unwrap().is_none(), "rolled back with the tabs");

        db.create_notes_with_tabs(&[note("n")], &[tab("t2", "n", 2), tab("t1", "n", 1)]).unwrap();
        let ids: Vec<String> = db.tabs_for("n").unwrap().into_iter().map(|t| t.id).collect();
        assert_eq!(ids, vec!["t1".to_string(), "t2".to_string()]);
        assert!(!db.tabs_for("n").unwrap()[0].created_at.is_empty(), "timestamps come from the database");
    }

    #[test]
    fn replacing_the_desktop_settings_keeps_the_plugins_keys() {
        let db = test_db();
        db.set_setting("theme", "dark").unwrap();
        db.set_setting("note.n.label.t", "Prices").unwrap();
        db.set_setting("onote.defaultFontSize", "18").unwrap();
        db.set_setting("markdownMirrorDir", "/tmp/vault").unwrap();
        db.set_setting("welcomeCreated", "1").unwrap();
        let mut fresh = std::collections::HashMap::new();
        fresh.insert("autoSave".to_string(), "1".to_string());
        // A doctored backup carrying the plugin's keys: ignored, not written over.
        fresh.insert("note.n.label.t".to_string(), "Imported".to_string());
        fresh.insert("onote.defaultFontSize".to_string(), "40".to_string());
        fresh.insert("markdownMirrorDir".to_string(), "/tmp/elsewhere".to_string());
        fresh.insert("welcomeCreated".to_string(), "0".to_string());
        db.replace_all_settings(&fresh).unwrap();
        assert_eq!(db.get_setting("theme").unwrap(), None, "the desktop's own key is replaced");
        assert_eq!(db.get_setting("autoSave").unwrap().as_deref(), Some("1"));
        for (k, v) in [("note.n.label.t", "Prices"), ("onote.defaultFontSize", "18"), ("markdownMirrorDir", "/tmp/vault"), ("welcomeCreated", "1")] {
            assert_eq!(db.get_setting(k).unwrap().as_deref(), Some(v), "{k} survives the desktop's reset and import");
        }
        db.set_all_settings(&fresh).unwrap();
        assert_eq!(db.get_setting("markdownMirrorDir").unwrap().as_deref(), Some("/tmp/vault"), "nor does a merge write them");
    }

    #[test]
    fn stacked_notes_tabs_are_left_out_of_the_open_listing() {
        let db = test_db();
        db.create_note(&note("open")).unwrap();
        db.create_note(&note("stacked")).unwrap();
        db.create_tab("t-open", "open").unwrap();
        db.create_tab("t-stacked", "stacked").unwrap();
        db.pile_note("stacked").unwrap();

        let open: Vec<String> = db.list_open_tabs().unwrap().into_iter().map(|t| t.id).collect();
        assert_eq!(open, vec!["t-open".to_string()]);
        let mut visited = Vec::new();
        db.for_each_open_tab(|t| {
            visited.push(t.id);
            Ok(())
        })
        .unwrap();
        assert_eq!(visited, open, "the streaming visitor sees the same rows");
        let err = db.for_each_open_tab(|_| Err("stop".to_string()));
        assert_eq!(err, Err("stop".to_string()), "a visitor error ends the walk");
        assert_eq!(db.tabs_for("stacked").unwrap().len(), 1, "fetched on restore");
        assert_eq!(db.list_tabs().unwrap().len(), 2, "exports still see every tab");
    }

    #[test]
    fn tab_changes_touch_the_parent_note() {
        let db = test_db();
        db.create_note(&note("a")).unwrap();
        db.create_note(&note("b")).unwrap();
        let old = "2000-01-01 00:00:00";
        let pin = || db.execute_for_tests(&format!("UPDATE notes SET updated_at = '{old}'"));
        let touched = |id: &str| db.get_note(id).unwrap().unwrap().updated_at != old;
        pin();
        let tab = db.create_tab("t", "a").unwrap();
        assert!(touched("a") && !touched("b"), "created");
        pin();
        db.update_tab(&NoteTab { content_blocks: "[]".into(), ..tab.clone() }).unwrap();
        assert!(touched("a") && !touched("b"), "edited");
        pin();
        db.delete_tab(&tab.id).unwrap();
        assert!(touched("a") && !touched("b"), "deleted");
    }

    #[test]
    fn deleting_all_notes_drops_every_notes_settings() {
        let db = test_db();
        db.create_note(&note("a")).unwrap();
        db.create_note(&note("b")).unwrap();
        let tab = db.create_tab("t", "a").unwrap();
        db.set_setting("note.a.title.main", "Groceries").unwrap();
        db.set_setting("note.a.label.t", "Prices").unwrap();
        db.set_setting("note.a.activeTab", &tab.id).unwrap();
        db.set_setting("note.b.titleLocked", "true").unwrap();
        db.set_setting("onote.columns", "2").unwrap();
        db.set_setting("theme", "keep").unwrap();
        db.delete_all_notes().unwrap();
        assert!(db.get_all_notes().unwrap().is_empty());
        assert!(db.list_tabs().unwrap().is_empty(), "tabs cascade");
        let mut keys: Vec<String> = db.get_all_settings().unwrap().into_keys().collect();
        keys.sort();
        assert_eq!(keys, ["onote.columns", "theme"], "only note.<id>.* keys are purged");
    }

    #[test]
    fn deleting_a_note_or_tab_drops_its_settings() {
        let db = test_db();
        db.create_note(&note("n")).unwrap();
        db.create_note(&note("other")).unwrap();
        let tab = db.create_tab("t", "n").unwrap();
        db.set_setting("note.n.title.main", "Groceries").unwrap();
        db.set_setting("note.n.titleLocked", "true").unwrap();
        db.set_setting("note.n.label.t", "Prices").unwrap();
        db.set_setting("note.n.columns.t", "2").unwrap();
        db.set_setting("note.other.label.t", "not ours").unwrap();
        db.set_setting("theme", "keep").unwrap();

        db.delete_tab(&tab.id).unwrap();
        let keys = |db: &Database| {
            let mut k: Vec<String> = db.get_all_settings().unwrap().into_keys().collect();
            k.sort();
            k
        };
        assert_eq!(keys(&db), ["note.n.title.main", "note.n.titleLocked", "note.other.label.t", "theme"]);

        db.delete_note_with_settings("n").unwrap();
        assert_eq!(keys(&db), ["note.other.label.t", "theme"]);
        assert!(db.get_note("n").unwrap().is_none());
    }
